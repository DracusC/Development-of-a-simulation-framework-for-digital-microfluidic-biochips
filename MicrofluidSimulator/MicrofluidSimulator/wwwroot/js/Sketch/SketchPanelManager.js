...
let sketch = function (p) {
    ...
    function getGroupVertexPairsForLerp(groupID, prevGroupID) {
        ...
        for (i in vertices_to_lerp) {
            let min_dist = [Infinity, "point"]; // min_dist, point
    ...
    //    if (!cur_group || !prev_group) { return []; }
    ...
    //    let vertices_to_lerp_copy = vertices_to_lerp.slice();

    //    // Iterate over the copy to prevent modification issues
    //    for (let i = 0; i < vertices_to_lerp_copy.length; i++) {
    //        let vertex = vertices_to_lerp_copy[i];
    //        let min_dist = Infinity;
    //        let closest_point = null;

    //        for (let j = 0; j < cur_group.vertices.length; j++) {
    //            let cur_vertex = cur_group.vertices[j];
    //            let dist = p.dist(vertex[0], vertex[1], cur_vertex[0], cur_vertex[1]);

    //            if (dist < min_dist && !processed_vertices.includes(cur_vertex)) {
    //                min_dist = dist;
    //                closest_point = cur_vertex;
    //            }
    //        }

    //        if (closest_point) {
    //            vertex_pairs.push([[vertex[0], closest_point[0]], [vertex[1], closest_point[1]]]);
    //            processed_vertices.push(closest_point);
    //        }
    //    }
